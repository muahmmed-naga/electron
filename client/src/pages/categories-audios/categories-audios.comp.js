...
import './categories-audios.styles.scss';

const CategoriesAudios = ({ match }) => {
  const { params } = match;
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    document.title = `Electron | Audios | Product ${params.id} Page`;

    setIsLoading(true);
    axios.get(`/api/v1/categories/audios/product/${params.id}`).then((res) => {
  ...
  return (
    <>
      <div className="custom-container categpries-audios-page-wrapper">
...
export default CategoriesAudios;
